...
public class TreeMatrixLayoutBuilder {
    ...
    protected Map<Date, MonthStatistics> monthStats;
    ...
    public TreeMatrixLayoutBuilder(CategoryDateTreeMatrix matrix, Map<Date, MonthStatistics> monthStats) {
        this.matrix = matrix;
    ...
    public void renderHeader() {
        int c = 0;
        for (Date month : matrix.getColumns()) {
            String label = format.format(month);
            int column = startMatrixColumn + c;
            excel.setCell(ROW_DATE, column, label);
            columns.put(month, column); // index
        ...
        for (CategoryDateTreeMatrixItem i : matrix.getTree()) {
            ...
            for (Date d : matrix.getColumns()) {
                ...
                if (count > 0) {
                    ...
                    // String label = opSum.apply(expenses) + " (" +
                    // opCount.apply(expenses) + ")";
    ...
    public void renderColumnStats() {
        if (monthStats == null)
            return;
        int rows = matrix.getTree().size() + 1;

        // month statistics
        int row = rows + 2;
        if (monthStats != null) {
            for (Date month : monthStats.keySet()) {
                int col = columns.get(month);

                MonthStatistics stat = monthStats.get(month);
                if (stat != null) {
                    Double solde = stat.get(MonthStat.SOLDE);
                    if(solde!=null)
                        excel.setCell(row + 0, col, solde);
                    Double score = stat.get(MonthStat.NUMBER_OF_NEGATIVE_DAYS);
                    if(score!=null)
                        excel.setCell(row + 1, col, score);
                    
                }
                else
                    System.err.println("missing month stats");
            }
        }
    }
